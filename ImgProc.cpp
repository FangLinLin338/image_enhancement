...
bool ImgEnhance::Test(const cv::Mat srcImg, cv::Mat &dstImg)
{
...
bool ImgEnhance::CLHE(const cv::Mat CtdL, cv::Mat &HdL, const float fClipRatio)
{
	CV_Assert(!CtdL.empty());
	std::vector<int> vnH;
	vnH.resize(256);
...
bool ImgEnhance::ModifyYComponent(const cv::Mat srcImg,	const cv::Mat oriL, const cv::Mat dstL, cv::Mat &dstImg)
{
	CV_Assert(!srcImg.empty());
...
bool ImgEnhance::SqrtAutoColorLevel(const cv::Mat srcImg, cv::Mat &dstImg)
{
	CV_Assert(!srcImg.empty());
...
bool ImgEnhance::LogTrans(const cv::Mat srcImg, cv::Mat &dstImg, const float fRatio)
{
	CV_Assert(!srcImg.empty());
...
bool ImgEnhance::NormLogL(const cv::Mat oriLogL, cv::Mat &dstL)
{
